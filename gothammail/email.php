...
<? 
	extract($_REQUEST);
	$result = mysql_query("SELECT * FROM emails WHERE email_id = $email_id") or die(mysql_error());
...
<table>
	...
	<?
		if (hasAttachments($email_id)) {
			...
			$result = mysql_query("SELECT * FROM email_bodies WHERE email_id = $email_id AND type != 'text' AND type != 'multipart' AND filename != ''") or die(mysql_error());
	...
	<?
		$result = mysql_query("SELECT * FROM email_bodies WHERE email_id = $email_id AND type = 'text'") or die(mysql_error());
		while($resultRow = mysql_fetch_array($result)) {
			extract($resultRow);
